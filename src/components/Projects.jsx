...
const ProjectModal = ({ project, isOpen, closeModal }) => (
  ...
  >
    <div className="bg-gray-900 p-6 rounded-lg shadow-2xl transform transition-transform duration-500 ease-out space-y-4 sm:space-y-0  max-w-md max-h-[80vh] overflow-y-auto">
      <button
        onClick={closeModal}
        className="absolute top-2 right-2 text-lightDesert hover:text-goldDesert"
      >
        <FaTimesCircle size={32} />
      </button>

      <div>
        <h3 className="text-lightDesert text-2xl sm:text-3xl font-bold mb-4">
          {project.title}
        </h3>
        <p className="text-lightDesert mb-4">{project.description}</p>
        <p className="text-lightDesert font-bold mb-4">{project.date}</p>
        <div className="flex justify-between items-center">
          {project.github ? (
            <a
              ...
              className="mx-2 text-lightDesert hover:text-goldDesert transition-colors duration-300"
            >
              <FaGithub size={32} />
            </a>
          ) : null}
...
export default function Projects() {
  ...
  return (
    <div
      id="projects"
      className="container mx-auto flex flex-col items-center p-8 bg-gray-900 mt-12 min-h-screen"
    >
      <h2 className="text-4xl font-bold text-lightDesert mb-4">Projects</h2>
